...
function Panel:mousepressed (x, y, b)
    ...
    if self.inHover then
        self.isActive = true
        if y < self.pos.y + 20 and b == 1 and not self.isOnResize then -- if is not resizing and the mouse is over tittle
            self.isGrabbed = true
            -- move the panel to the mouse position (considerating pivot)
            self:setPosition(
                love.mouse.getX() - (self.size.x * self.pivot.x),
                love.mouse.getY() - (self.size.y * self.pivot.y)
            )
        ...
        if y > self.pos.y + self.size.y - 10 and x > self.pos.x + self.size.x - 10 and b == 1  and not self.isGrabbed then
